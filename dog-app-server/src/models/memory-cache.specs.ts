...
import { Settings } from '../../dist/types/app-settings';
import { Breed } from '../../dist/types/breed';
import { AllValue } from '../../dist/types/interfaces/model';
import { converPageAndPageSizeToStartAndEndFormat, serializeMemoryCacheRangeRequestKey } from '../../dist/utils/model-utils';
import { SortDir } from '../../dist/types/data';

export function createCacheKey(entityId = 'breed', page = 1, pageSize = 10, sortBy = 'id', sortDir: SortDir = 'desc') {
	const params = converPageAndPageSizeToStartAndEndFormat(page, pageSize);
	return serializeMemoryCacheRangeRequestKey(entityId, params[0], params[1], sortBy, sortDir);
}
...
describe('MemoryCache', () => {
	...
	Settings.cacheCleaningFrequency = 1000;
	Settings.cacheLimit = 2;
	Settings.cacheExpiresAfterMinutes = 3 * 60;
	Settings.cacheOldItemsThresholdHours = 1;
	...
	describe('registerConnector', () => {
		it('should register a DataConnector', async () => {
			...
			sinon.assert.calledOnce(dataConnector.getRange);
		})
	})
	...
	describe('read', () => {
		...
		it('remove items that expired and try to retrieve them again from the connector', async () => {
			...
			dataConnector.getRange.resolves(mockData)
			await cache.read(cacheId);
			...
			clock.tick(Settings.cacheCleaningFrequency);

			sinon.assert.calledTwice(dataConnector.getRange);
		})
		...
		it('remove elements older than two hours from the cache if its limit was reached', async () => {
			...
			dataConnector.getRange.onFirstCall().resolves(mockData1);
			dataConnector.getRange.onSecondCall().resolves(mockData2);
			dataConnector.getRange.onThirdCall().resolves(mockData3);

			// add first item to cache
			await cache.read(createCacheKey(entityId, 1));
			expect(dataConnector.getRange.callCount).to.equal(1);
			...
			expect(dataConnector.getRange.callCount).to.equal(2);
			...
			expect(dataConnector.getRange.callCount).to.equal(3);

			// wait for cache cleaning interval
			clock.tick(Settings.cacheCleaningFrequency + 1000);

			// try to read the first item again, it should have been removed and require a new fetch
			await cache.read(createCacheKey(entityId, 1));
			expect(dataConnector.getRange.callCount).to.equal(4);
		...
		it('should handle range requests correctly', async () => {
			...
			expect(dataConnector.getRange.calledOnce).to.be.true;
		});

		it('should handle errors from data connector gracefully', async () => {
			dataConnector.getSingle.rejects(new Error('Database error'));
			...
			try {
				...
				expect(error.message).to.equal(`Error in the Data connector associated to the entity ${entityId}`);
			}
		});
	})
